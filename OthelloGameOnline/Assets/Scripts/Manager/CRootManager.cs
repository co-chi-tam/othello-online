...
public class CRootManager: CMonoSingleton<CRootManager> {
	...
	{
		// TRANSFORM
		this.m_MainCanvas = GameObject.FindGameObjectWithTag ("Main");
		this.m_TopCanvas = GameObject.FindGameObjectWithTag ("Top");
		// CONFIGS
	...
	public virtual CDefaultScene ShowScene(string sceneName, bool savePath = true)
	{
			...
			&& this.m_CurrentScene.sceneObjectName == sceneName)
		{
			...
			this.m_CurrentScene.GameObject.transform.localPosition = Vector3.zero;
		...
		foreach(var item in this.m_SceneMaps)
		{
				...
				&& item.Key == sceneName)
			{
				...
				item.Value.GameObject.transform.localPosition = Vector3.zero;
	...
	public virtual CDefaultPopup ShowPopup(string popupName, bool savePath = true)
	{
			...
			&& this.m_CurrentPopup.sceneObjectName == popupName)
		{
			...
			this.m_CurrentPopup.GameObject.transform.localPosition = Vector3.zero;
		...
		if (this.m_PopupMaps.ContainsKey(popupName))
		{
			...
			this.m_PopupMaps[popupName].GameObject.transform.localPosition = Vector3.zero;
