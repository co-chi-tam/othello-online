...
public class CSetingUserScenePanel : CDefaultScene {
	...
	public override void OnInitObject()
	{
		...
		// EVENTS
		CSocketManager.Instance.On("receivePlayerData", this.ReceivePlayerData);
	...
	private void OnSubmitClick()
	{
		...
		// SET PLAYER NAME
		this.SetPlayerInfo(CGameSetting.USER_NAME, CGameSetting.USER_AVATAR);
	}

	private void ReceivePlayerData (SocketIO.SocketIOEvent ev) 
	{
		// UI
		// this.m_SubmitButton.interactable = true;
		// this.m_PlayerNameInputField.interactable = true;
		...
		this.EnableGridAvatars(true);
		// SWITCH SCENE
		CRootManager.Instance.ShowScene("LobbyDisplayPanel");
	}
	...
	public void SetPlayerInfo(string playerName, int avatarIndex) 
	{
		...
		// EMIT
		CSocketManager.Instance.Emit("setPlayerData", userData);
	}
