...
public class CShopPopup : CDefaultPopup {
	...
	public override void OnInitObject()
	{
		...
		// ITEM PREFAB
		this.m_ShopItemPrefab = Resources.Load<CShopItem>("UnityPopups/Shop/ShopItem");
		this.m_ShopConfig = Resources.Load<TextAsset>("UnityPopups/Shop/shopConfig");
	...
	private void LoadConfig(string config)
	{
		...
		for (int i = 0; i < shopItems.Length; i++)
		{
			...
			// }
			// else 
			if (item.type == CShopItem.EShopType.AVATAR)
			{
	...
	private void OnItemClick(int value)
	{
		...
		if (CGameSetting.USER_GOLD >= shopItem.price)
		{
			confirm.Show("CONFIRM", "Do you want buy item ?", "OK", () => {
				...
				// }
				// else 
				if (shopItem.type == CShopItem.EShopType.AVATAR)
				{
	...
	private void OnSubmitData()
	{
		...
		{
			CGameSetting.UpdatePlayerData(CGameSetting.USER_NAME, CGameSetting.USER_AVATAR);
		}
	}
